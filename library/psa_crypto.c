...
psa_status_t psa_destroy_key(mbedtls_svc_key_id_t key)
{
    ...
    if (psa_key_lifetime_is_external(slot->attr.lifetime))
    {
            ...
            PSA_KEY_LOCATION_IFX_SE)
        {
            ...
            /* Key is located on SE RT Services */
            ...
            (operation->ctx.ifx_se_op.operation_handle != 0u))
        {
            ...
            status = ifx_se_get_psa_status(
                        ifx_se_key_derivation_out_key(
                            &se_attributes, &operation->ctx.ifx_se_op,
                            ifx_se_fih_ptr_encode(&se_key), IFX_SE_NULL_CTX));
            if (status == PSA_SUCCESS)
            {
                /* Copy new key ID to the key material, because for the keys
                   stored in SE we store an ID value on the host side
                   instead of storing actual key material.
                */
                status = psa_copy_key_material_into_slot(
                            slot, (const uint8_t *)&se_key, sizeof(se_key));
            }
        }
        else
#endif /* MBEDTLS_PSA_CRYPTO_DRIVERS && IFX_PSA_SE_DPA_PRESENT */
