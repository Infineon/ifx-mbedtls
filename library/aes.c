...
/* TODO: IFX_AES_ECB_USE_HARDWARE added only because MBEDTLS_AES_USE_HARDWARE_ONLY does not work properly */
#if defined(IFX_AES_ECB_USE_HARDWARE)
#include "psa_crypto_driver_wrappers.h"
#endif /* IFX_AES_ECB_USE_HARDWARE */
 ...
 */
#if !defined(MBEDTLS_AES_ENCRYPT_ALT) && !defined(IFX_AES_ECB_USE_HARDWARE)
...
#endif /* !MBEDTLS_AES_ENCRYPT_ALT && !IFX_AES_ECB_USE_HARDWARE */

/*
 * AES-ECB block decryption
 */
#if !defined(MBEDTLS_AES_DECRYPT_ALT) && !defined(IFX_AES_ECB_USE_HARDWARE)
...
#endif /* !MBEDTLS_AES_DECRYPT_ALT && !IFX_AES_ECB_USE_HARDWARE */
                          ...
                          unsigned char output[16])
{
    ...
    if (mode == MBEDTLS_AES_ENCRYPT) {
        ...
        size_t iv_size = 0;
        return psa_driver_wrapper_cipher_encrypt(&attributes, (uint8_t *)(ctx->buf + ctx->rk_offset),
                                                 key_size, PSA_ALG_ECB_NO_PADDING,
                                                 local_iv, iv_size, input, io_size,
                                                 output, io_size, &output_length);
    } else {
        return psa_driver_wrapper_cipher_decrypt(&attributes, (uint8_t *)(ctx->buf + ctx->rk_offset),
                                                 key_size, PSA_ALG_ECB_NO_PADDING, input, io_size,
                                                 output, io_size, &output_length);
    }

#elif !defined(MBEDTLS_AES_USE_HARDWARE_ONLY)
    if (mode == MBEDTLS_AES_ENCRYPT) {
