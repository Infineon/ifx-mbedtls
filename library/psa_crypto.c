...
psa_status_t psa_destroy_key(mbedtls_svc_key_id_t key)
{
    ...
    if (psa_key_lifetime_is_external(slot->attr.lifetime))
    {
            ...
            PSA_KEY_LOCATION_IFX_SE)
        {
            ...
            (void)memcpy(&se_key_id, slot->key.data, sizeof(se_key_id));
                              ...
                              size_t hash_length )
{
    ...
    psa_status_t status = psa_driver_wrapper_hash_verify(operation, hash, hash_length);
                                    ...
                                    size_t *iv_length)
{
    ...
    status = psa_driver_wrapper_cipher_generate_iv( operation,
                                               local_iv, default_iv_length, iv_length );
exit:
    if (status == PSA_SUCCESS) {
                                     ...
                                     size_t *nonce_length)
{
    ...
    *nonce_length = 0;
    ...
    status = psa_driver_wrapper_aead_generate_nonce(operation, local_nonce,
                                                required_nonce_size, nonce_length);
exit:
    if (status == PSA_SUCCESS) {
        memcpy(nonce, local_nonce, *nonce_length);
    } else {
        psa_aead_abort(operation);
    }
            ...
            (operation->ctx.ifx_se_op.operation_handle != 0u))
        {
            ...
            status = ifx_se_get_psa_status(
                        ifx_se_key_derivation_out_key(
                            ...
                            ifx_se_fih_ptr_encode(&fih_key_id), IFX_SE_NULL_CTX));
            if (status == PSA_SUCCESS)
            {
                // TODO: get OWNER from original key in context
                ...
                */
                status = psa_copy_key_material_into_slot(
                            slot, (const uint8_t *)&se_key_id, sizeof(se_key_id ));
                if (status == PSA_SUCCESS)
                {
                    *key = se_key_id;
                }
    ...
    size_t data_length)
{
    ...
    if ((operation->alg == PSA_ALG_KDF_IFX_SE_AES_CMAC) &&
        (operation->ctx.ifx_se_op.operation_handle != 0u))
    {
        psa_status_t status = ifx_se_get_psa_status(
...
#else /* MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG */
	/* Need to check if these iterations are required */
    while (output_size > 0) {
        size_t request_size =
